  ...
  class StandardRenderer
    ...
    def render_crumbs(crumbs, options = {})
      ...
      when :html_list
        ...
        crumb_string = crumbs.collect do |crumb|
          crumb_to_html_list(crumb, options[:links], options[:separator], options[:li_class], options[:active_li_class], options[:first_class], options[:last_class], (crumb == crumbs.first), (crumb == crumbs.last))
    ...
    def crumb_to_html_list(crumb, links, separator, li_class, active_li_class, first_class, last_class, is_first, is_last)
      name, url = crumb
      html_classes = []
      html_classes << first_class if is_first
      html_classes << last_class if is_last
      ...
      content_tag(:li, (url && links ? link_to(name, url) : content_tag(:span, name)) + (separator.html_safe if !is_last), :class => html_classes.join(' ').strip)
    end
